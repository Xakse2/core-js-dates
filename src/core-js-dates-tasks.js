...
function getNextFriday(date) {
  ...
  const daysNeed = (5 - day + 7) % 7 || 7;
  result.setUTCDate(result.getUTCDate() + daysNeed);
  return result;
}
...
function formatDate(date) {
  ...
  const period = data.getUTCHours() >= 12 ? 'PM' : 'AM';
  const time = `${data.getUTCHours() % 12 || 12}:${data.getUTCMinutes().toString().padStart(2, '0')}:${data.getUTCSeconds().toString().padStart(2, '0')} ${period}`;
  return `${data.getUTCMonth() + 1}/${data.getUTCDate()}/${data.getUTCFullYear()}, ${time}`;
}
